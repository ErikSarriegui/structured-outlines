...
## 📂 Project Structure
* `json_schema.py`: Pydantic to Regex conversion (~150 lines).
* `regex_parser.py`: Custom Regex to DFA engine (~370 lines).
* `guide.py`: DFA-based token transition mapping (~80 lines).
* `generate.py`: Hugging Face integration and Logits processing (~90 lines).
