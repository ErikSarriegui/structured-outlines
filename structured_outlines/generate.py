...
class StructuredLogitsProcessor:
    ...
    Designed for ``batch_size == 1`` (greedy / sampling).  For beam search,
    only the first beam is constrained — a deliberate simplification.
    """
    ...
    def __call__(
        ...
        if not valid:
            # No valid continuation — should not happen with a correct regex.
            return scores
