...
class RegexGuide:
    ...
    def __init__(self, pattern: str, tokenizer, *, verbose: bool = False):
        self.dfa: DFA = regex_to_dfa(pattern)
    ...
    def _precompute(self, tokenizer, *, verbose: bool = False):
        ...
        for state in range(n_states):
            ...
            # Only consider tokens whose first char has a valid transition.
            valid_first = self.dfa.transitions.get(state, {})
            for first_ch in valid_first:
                for tid, tok_str in by_first.get(first_ch, []):
